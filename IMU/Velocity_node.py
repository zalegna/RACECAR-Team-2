...
class VelocityNode(Node):
    def __init__(self):
        ...
        self.publisher_velocity = self.create_publisher(Vector3, '/velocity', 10) # velcoti
    ...
    def imu_callback(self, data):
            ...
            if current_lidar_corners is not None and st is not None:
                ...
                if len(good_new) >= 1:
                    ...
                    if abs(np.sum(self.mean_disp)) > .0001:
                        ...
                        # print(f"lvelocityy: {round(lidar_velocity_y, 2)}")
                        # print("\n")

                        # clamp values to reasonable ones
                        max_speed = 2.5

                        if lidar_velocity_x > max_speed:
                            lidar_velocity_x = max_speed
                        elif lidar_velocity_x < -max_speed:
                            lidar_velocity_x = -max_speed

                        if lidar_velocity_y > max_speed:
        ...
        # comp filter
        # both of these steps are done inside the lidar section above, since lidar updates slower than IMU and itd cause errors
        ...
        if np.sum(self.mean_disp) != 0:
            print(f"x: {round(self.x_velocity, 2)}")
            print(f"y: {round(self.y_velocity, 2)}")
            #print(f"x: {self.z_velocity}")
            # print(type(self.x_velocity))
            # print(type(self.y_velocity))
            # print(type(self.z_velocity))
            velocity = Vector3()
