...
Purpose: Wall follow through the grand prix course. Prints telemetry info to the terminal (lidar dist, current state, etc.)
"""
...
SUPER_GREEN = ((38, 132, 1), (80, 255, 255))
RED = ((174, 132, 1), (179, 255, 255))
COLOR_PRIORITY = (SUPER_GREEN, GREEN, RED, ORANGE)
...
def get_lidar_visualization(scan, sim):
    ...
    for xi, yi in zip(x_img, y_img):
        cv2.circle(img, (xi, yi), radius=1, color=(255, 255, 255), thickness=-1)  # white

    # Draw robot at center (red dot)
    #cv2.circle(img, (120, 120), radius=4, color=(0, 0, 255), thickness=-1)  # red
...
def elevator_align():
    ...
    cropped = rc_utils.crop(image, (100, 0), (rc.camera.get_height(), rc.camera.get_width()))
    hsv = cv2.cvtColor(cropped, cv2.COLOR_BGR2HSV)
    mask = cv2.inRange(hsv, SUPER_GREEN[0], SUPER_GREEN[1])
    contours, _ = cv2.findContours(mask, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    if len(contours) == 0:
        return False
    largest = max(contours, key=cv2.contourArea)
    ...
    if center is None:
        return False
    setpoint = rc.camera.get_width() / 2 + 35
    error = setpoint - center[1]
    angle = rc_utils.clamp(-0.00315 * error, -1, 1)
    speed = 0.65
...
def update():
    ...
    scan_data = rc.lidar.get_samples()
    lidar_img=get_lidar_visualization(scan_data, False)
...
def bad_wall():
    ...
    r_angle, r_dist = rc_utils.get_lidar_closest_point(scan_data, (0, 180))
    l_angle, l_dist = rc_utils.get_lidar_closest_point(scan_data, (-30, 0))
    r_shift = rc_utils.get_lidar_average_distance(scan_data, special_light, sample_window)
    l_shift = rc_utils.get_lidar_average_distance(scan_data, -20, sample_window)
